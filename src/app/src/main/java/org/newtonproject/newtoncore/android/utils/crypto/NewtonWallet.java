...
public class NewtonWallet {
    ...
    private static NewtonWalletFile create(String password, String cipherText, String derivePath, String type, int n, int p)
        ...
        if(type != null && type.equals(NewtonWalletFile.FILETYPE_MNEMONIC)) {
            // mnemonic type
            privateKeyBytes = cipherText.getBytes();
            ArrayList<String> mnemonicFromString = StringUtil.getMnemonicFromString(cipherText);
            ...
            BigInteger privateKey = keyChain.getKeyByPath(HDUtils.parseSupportPath(derivePath), true).getPrivKey();
            keyPair = ECKeyPair.create(privateKey);
        } else if(type != null && type.equals(NewtonWalletFile.FILETYPE_PRIVATE)) {
            // private type
            privateKeyBytes = Numeric.hexStringToByteArray(cipherText);
            keyPair = ECKeyPair.create(privateKeyBytes);
        }
        String address = Keys.getAddress(keyPair);
        byte[] cipherTextBytes = performCipherOperation(
                Cipher.ENCRYPT_MODE, iv, encryptKey, privateKeyBytes);

        byte[] mac = generateMac(derivedKey, cipherTextBytes);

        return createNewtonWalletFile(address, cipherTextBytes, iv, salt, mac, n, p, derivePath, type);
    }
    ...
    public static String decrypt(String password, NewtonWalletFile NewtonWalletFile)
        ...
        type = type == null ? org.newtonproject.newtoncore.android.utils.crypto.NewtonWalletFile.FILETYPE_PRIVATE : type;
        ...
        if (!Arrays.equals(derivedMac, mac)) {
            throw new CipherException(StringUtil.getString(org.newtonproject.newtoncore.android.R.string.password_error));
        }
        String res = null;
        byte[] encryptKey = Arrays.copyOfRange(derivedKey, 0, 16);
        byte[] privateKey = performCipherOperation(Cipher.DECRYPT_MODE, iv, encryptKey, cipherText);
        if(type.equals(org.newtonproject.newtoncore.android.utils.crypto.NewtonWalletFile.FILETYPE_MNEMONIC)){
            res = new String(privateKey);
        }else if(type.equals(org.newtonproject.newtoncore.android.utils.crypto.NewtonWalletFile.FILETYPE_PRIVATE)){
    ...
    private static String getNewId(ECKeyPair keyPair) {
        BigInteger publicKey = keyPair.getPublicKey();
        String pubStr = Numeric.toHexStringWithPrefixZeroPadded(publicKey, 128);
        String publicKeyNoPrefix = Numeric.cleanHexPrefix(pubStr);
        if (publicKeyNoPrefix.length() < 128) {
            publicKeyNoPrefix = Strings.zeros(128 - publicKeyNoPrefix.length()) + publicKeyNoPrefix;
        }
        String hash = Hash.sha3(publicKeyNoPrefix);
        return Numeric.cleanHexPrefix(hash).substring(0, 40);
    }
}
